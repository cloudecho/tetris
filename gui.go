...
func createRightPanel(parent *gtk.Box) {
	...
	grid, _ := gtk.GridNew()
	grid.Attach(grid0, 1, 0, 1, 1)
	grid.Attach(separator1, 1, 1, 1, 1)
	grid.Attach(scoreLabel, 1, 2, 1, 1)
	grid.Attach(scoreValue, 1, 3, 1, 1)
	grid.Attach(separator2, 1, 4, 1, 1)
	grid.Attach(levelLabel, 1, 5, 1, 1)
	grid.Attach(levelValue, 1, 6, 1, 1)
	grid.Attach(separator3, 1, 7, 1, 1)
